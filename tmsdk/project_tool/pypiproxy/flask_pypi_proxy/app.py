...
def read_configuration(app, pypi_url='http://pypi.python.org',
    ...
    if filepath:
        ...
        with open(filepath) as config_file:
            ...
            if 'LOGGING_PATH' in configuration:
...
def configure_logging(app:Flask):
    ...
    '''
    logFilePath = os.path.join(os.getcwd(),'server.log')
    loglevel = logging.INFO

    if 'LOGGING_PATH' in app.config:
        logFilePath = app.config['LOGGING_PATH']
    if 'LOGGING_LEVEL' in app.config:
        loglevel = getattr(logging,app.config['LOGGING_LEVEL'])
    logFH = logging.FileHandler(filename=logFilePath)
    logFH.setLevel(logging.DEBUG)
    logSH = logging.StreamHandler(sys.stdout)
    logSH.setLevel(loglevel)
    logging.basicConfig(format='%(asctime)s [%(levelname)s] %(message)s',level=logging.DEBUG,handlers=[logFH,logSH])
