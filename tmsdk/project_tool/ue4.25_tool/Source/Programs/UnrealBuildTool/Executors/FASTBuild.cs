...
namespace UnrealBuildTool
{
	...
	class FASTBuild : ActionExecutor
	{
		...
		private readonly static Tuple<string, Func<Action, string>, FBBuildType>[] BuildTypeSearchParams = new Tuple<string, Func<Action, string>, FBBuildType>[]
		{
			...
			(
				Path.Combine("toolchains","llvm","prebuilt"),	// ndk clang++ path , Android not a great test
		...
		public override bool ExecuteActions(List<Action> Actions, bool bLogDetailedActionStats)
		{
			...
			Log.TraceInformation(Actions[0].CommandPath.FullName);
			Log.TraceInformation(Actions[0].CommandArguments);
			Log.TraceInformation("======================================");
			IEnumerable<Action> CompileActions		= Actions.Where(Action => (Action.ActionType == ActionType.Compile && Action.bCanExecuteRemotelyWithSNDBS == true));
			IEnumerable<Action> NonCompileActions	= Actions.Where(Action => (Action.ActionType != ActionType.Compile || Action.bCanExecuteRemotelyWithSNDBS == false));
		...
		private Dictionary<string, string> ParseCommandLineOptions(string CompilerCommandLine, string[] SpecialOptions, bool SaveResponseFile = false)
		{
			...
			//fix: 
			//if (ResponseFileTokenIndex = 0) => if (ResponseFileTokenIndex >= 0)
			...
			for (int i = 0; i < ProcessedTokens.Count; ++i)
			{
				...
				if ((Token == "/I")						||
					...
					(Token == "-isysroot")				||
					(Token == "-include")				|| 
					(Token == "-include-pch") ||     //android ndk 编译有出现	
		...
		private void WriteEnvironmentSetup()
		{
			...
			//这是windows打windows包头
			//BuildHostPlatform.Current.Platform == UnrealTargetPlatform.Windows
			if (VCEnv != null && targetDescriptor.Platform == UnrealTargetPlatform.Win64)
			{
				...
				AddText($"Print('clui=====$CLFilterRoot$\\2052\\clui.dll')\n");
			...
			if (BuildHostPlatform.Current.Platform == UnrealTargetPlatform.Mac && (targetDescriptor.Platform == UnrealTargetPlatform.Mac || targetDescriptor.Platform == UnrealTargetPlatform.IOS || targetDescriptor.Platform == UnrealTargetPlatform.TVOS))
			{
            ...
            //Android Support
            if (targetDescriptor.Platform == UnrealTargetPlatform.Android)
            {
		...
		private void AddCompileAction(Action Action, IEnumerable<Action> DependencyActions)
		{
			...
			else if (Action.CommandPath.FullName.Contains("clang++"))
			{
				CompilerName = "clang++";
			}
			else if (Action.CommandPath.FullName.Contains("clang"))
			...
			string[] SpecialCompilerOptions = { "/Fo", "/fo", "/Yc", "/Yu", "/Fp", "-o", "-dependencies=", "-compiler=","-M","-MF" };
			...
			IntermediatePath = LocalizationDirSep(IntermediatePath);
			...
			//string outputFile = GetOptionValue(ParsedCompilerOptions, "-o", Action, ProblemIfNotFound: true);
			...
			var outputFileName = Path.GetFileName(OutputObjectFileName);
			var inputFileExt = Path.GetExtension(InputFile);
			CompilerOutputExtension = outputFileName.Replace(inputFileName, inputFileExt);
			...
			else
			{
				...
				else
				{
                    ...
                    {
						string sysRootInclude = "-I\"$NDKToolchainSysRoot$\\include\"";
						AddText($"\t.CompilerOptions = '{sysRootInclude} {OtherCompilerOptions} -D __BUILDING_WITH_FASTBUILD__ -fno-diagnostics-color -o \"%2\" \"%1\" '\n");
						//string InputFileExt = Path.GetExtension(InputFile);
   ...
   //         {
			...
			//	CreateBffFile_CompileAction(Action, DependencyActions);
            ...
            try
			{
				...
				switch (BuildType)
				{
					case FBBuildType.Windows:
						ret = CreateBffFile_Windows(Actions, BffFilePath);
						break;
					case FBBuildType.Apple:
						ret = CreateBffFile_Mac(Actions, BffFilePath);
						break;
					case FBBuildType.Android:
						ret = CreateBffFile_Android(Actions, BffFilePath);
						break;
					default:
						break;
				}
		...
		private void CreateBffFile_CompileAction(Action action, IEnumerable<Action> dependencyActions)
        {
			var exta = new ExtAction(action);
            switch (exta.compilerType)
            {
                case CompilerType.MSVC:
					CreateBffFile_CompileAction_MSVC(exta, dependencyActions);
                    break;
                case CompilerType.Clang:
					CreateBffFile_CompileAction_Clang(exta, dependencyActions);
                    break;
				case CompilerType.GCC:
					CreateBffFile_CompileAction_GCC(exta, dependencyActions);
                    break;
				default:
                    break;
            }
		...
		private void CreateBffFile_CompileAction_MSVC(ExtAction action, IEnumerable<Action> dependencyActions)
        {
			//CLFilterParams = "-dependencies=\"%CLFilterDependenciesOutput\" -compiler=\"%5\" -stderronly -- \"%5\" ";
			string newCLFilterArgs = "";
			if (action.useCLFilter && !noCLFilter)
            {
				var dependencies = UnpackQuotation(action.GetValueFromCommandArgs(ref action.rawAction.CommandArguments, "-dependencies="));
				newCLFilterArgs = $"-dependencies=\"{dependencies}\" -compiler=\"{action.compilerPath}\" -stderronly -- {action.compilerPath}";
                //newCLFilterArgs = "-dependencies=\"%CLFilterDependenciesOutput\" -compiler=\"%5\" -stderronly -- \"%5\"";
			}
			...
			string ShowIncludesParam = "/showIncludes";
			//原程序会移除这些项
			//string[] SpecialCompilerOptions = { "/Fo", "/fo", "/Yc", "/Yu", "/Fp", "-o", "-dependencies=", "-compiler=", "-M", "-MF" };
			//if (action.compilerArgs.Contains("/Yc")) //Create PCH
			if (action.isPCH)
            {
				string pchRawArgs = action.noInputOutputFileArgs;

				string PCHIncludeHeader = action.GetValueFromCommandArgs(ref pchRawArgs, "/Yc", noQuotation: true, removeIt: true);
				string _ = action.GetValueFromCommandArgs(ref pchRawArgs, "/Yu", noQuotation: true, removeIt: true);
				string PCHOutputFile = action.GetValueFromCommandArgs(ref pchRawArgs, "/Fp", noQuotation: true, removeIt: true);
				AddText($"\t.CompilerOptions = '{newCLFilterArgs} \"%1\" /Fo\"%2\" /Fp\"{PCHOutputFile}\" /Yu\"{PCHIncludeHeader}\" {pchRawArgs} '\n");

				AddText($"\t.PCHOptions = '{newCLFilterArgs} \"%1\" /Fp\"%2\" /Yc\"{PCHIncludeHeader}\" {pchRawArgs} /Fo\"{action.outputFilePath}\"'\n");
				AddText($"\t.PCHInputFile = \"{PCHIncludeHeader}\"\n");
				AddText($"\t.PCHOutputFile = \"{PCHOutputFile}\"\n");
			}
            else
            {
				AddText($"\t.CompilerOptions = '{newCLFilterArgs} {action.noInputOutputFileArgs} /Fo\"%2\" \"%1\" {ShowIncludesParam} '\n");
			}
            //else if (ParsedCompilerOptions.ContainsKey("/Yu")) //Use PCH
            //{
            //    string PCHIncludeHeader = GetOptionValue(ParsedCompilerOptions, "/Yu", Action, ProblemIfNotFound: true);
		...
		private void CreateBffFile_CompileAction_Clang(ExtAction action, IEnumerable<Action> dependencyActions)
		{

		}
		private void CreateBffFile_CompileAction_GCC(ExtAction action, IEnumerable<Action> dependencyActions)
		{
        ...
        class ExtAction
        {
            ...
            public ExtAction(Action action)
            {
				...
				if (compilerPath == "/bin/sh")
                {
					...
					realCmd = UnpackQuotation(splitArgs[1].Trim());
                }
				...
				//加载response file
				var responseFilePath = UnpackQuotation(GetValueFromCommandArgs(ref action.CommandArguments, "@"));
                if (string.IsNullOrEmpty(responseFilePath))
                {
                    Log.TraceInformation($"READ RESPONSEFILE = null ,args = {RawCommandArgs}");
                ...
                compilerArgs = File.ReadAllText(responseFilePath);
				if (compilerType == CompilerType.MSVC)
                {
					compilerArgs = compilerArgs.Replace(Environment.NewLine, " ");
				}

				inputFilePath = action.PrerequisiteItems[0].FullName;
				outputFilePath = action.ProducedItems[0].FullName;
				...
				noInputOutputFileArgs = compilerArgs;
                if (compilerType == CompilerType.MSVC)
                {
                ...
                {
					if (Path.GetExtension(outputFilePath) == ".gch")
					{
						isPCH = true;
						outputFileExt = ".h.gch";
					}
			...
			private bool IsCommandOptionFlag(char c)
            {
				return c == '/' || c == '-';
			}
			public string GetValueFromCommandArgs(ref string args, string key, bool noQuotation = false, bool removeIt = false)
			{
			...
			private string GetValueFromCommandArgs_Internal_NoCheck(string args, string key, int keyindex, bool isUseSpaceSplitKeyValue)
			{
		...
		public static string UnpackQuotation(string str)
		{
			...
			if (strSB[0] == '\"' && strSB[strSB.Length - 1] == '\"')
			{
		...
		bool noCLFilter = true;
		private bool CreateBffFile_Windows(IEnumerable<Action> Actions, string BffFilePath)
		{
			...
			envs.Add(string.Format("\t\t\"PATH={0}\\Common7\\IDE\\;{1}\",\n", VCEnv.GetVCInstallDirectory(), VCEnv.GetToolPath()));
		...
		private bool CreateBffFile_Mac(IEnumerable<Action> Actions, string BffFilePath)
		{
			//这是mac打mac、ios、tvos包头
		...
		private bool CreateBffFile_Android(IEnumerable<Action> Actions, string BffFilePath)
		{
			//Android Support
			var toolChain = AndroidExports.CreateToolChain(targetDescriptor.ProjectFile) as AndroidToolChain;
