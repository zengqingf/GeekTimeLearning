...
public class StringHelper
{
    ...
    {
        return Encoding.UTF8.GetString(bytes).TrimEnd(default_trim);
    ...
    public static bool IsAvailableString(string str)
    {
        ...
        while (num2 < length)
        {
            ...
            if (flag)
            {
                ...
                {
                    Logger.Log(string.Format("invalid utf-16 sequence at {0} (missing surrogate tail)", num2));
                    return false;
                }
                flag = false;
            }
            else
            {
                ...
                else
                {
                    if ((ch >= 0xdc00) && (ch <= 0xdfff))
                    {
                        Logger.Log(string.Format("invalid utf-16 sequence at {0} (missing surrogate head)", num2));
                        return false;
                    }
                    num += 3;
                }
