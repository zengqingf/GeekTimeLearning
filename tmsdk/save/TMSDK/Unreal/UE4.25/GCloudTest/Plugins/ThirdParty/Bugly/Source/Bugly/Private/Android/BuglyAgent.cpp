...
jmethodID FBuglyAgent::postLogError;
...
FBuglyAgent::FBuglyAgent()
{
	if (JNIEnv* Env = FAndroidApplication::GetJavaEnv())
	{
		...
		//postLogError = FJavaWrapper::FindMethod(Env, FJavaWrapper::GameActivityClassID, "AndroidThunkJava_Bugly_postLogError", "(I;Ljava/lang/String;Ljava/lang/String;Ljava/lang/String;Z)V", false);
		postLogError = FJavaWrapper::FindMethod(Env, FJavaWrapper::GameActivityClassID, "AndroidThunkJava_Bugly_postLogError", "(Ljava/lang/String;)V", false);
...
void FBuglyAgent::PostLogError( FString& name, FString& reason, FString& stackTrace)
{
	UE_LOG(LogTemp, Error, TEXT("Test error a %s %s"), *reason, *stackTrace);
	if (JNIEnv* Env = FAndroidApplication::GetJavaEnv())
	{
		jstring s1 = Env->NewStringUTF(TCHAR_TO_UTF8(*name));
		jstring s2 = Env->NewStringUTF(TCHAR_TO_UTF8(*reason));
		jstring s3 = Env->NewStringUTF(TCHAR_TO_UTF8(*stackTrace));

		FJavaWrapper::CallVoidMethod(Env, FJavaWrapper::GameActivityThis, postLogError, s3);
