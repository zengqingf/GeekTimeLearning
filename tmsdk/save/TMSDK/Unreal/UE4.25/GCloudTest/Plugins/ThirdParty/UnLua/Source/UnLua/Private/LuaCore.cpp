...
int32 Global_PrintWarning(lua_State* L)
{
    FString StrLog;
    int32 nargs = lua_gettop(L);
    for (int32 i = 1; i <= nargs; ++i)
    ...
    UE_LOG(LogUnLua, Warning, TEXT("== UNLUA_PRINT : %s"), *StrLog);
    if (IsInGameThread())
    {
        UKismetSystemLibrary::PrintString(GWorld, StrLog, false, false);
    }
...
int32 Global_PrintError(lua_State* L)
{
    FString StrLog;
    int32 nargs = lua_gettop(L);
    for (int32 i = 1; i <= nargs; ++i)
    ...
    UE_LOG(LogUnLua, Error, TEXT("== UNLUA_PRINT : %s"), *StrLog);
    if (IsInGameThread())
    {
        UKismetSystemLibrary::PrintString(GWorld, StrLog, false, false);
    }
