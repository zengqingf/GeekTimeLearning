...
void FLuaContext::CreateState()
{
    ...
    if (!L)
    {
        ...
        lua_register(L, "UEPrintWarning", Global_PrintWarning);
        lua_register(L, "UEPrintError", Global_PrintError);
...
bool FLuaContext::TryToBindLua(UObjectBaseUtility *Object)
{
    ...
    if (!Object->HasAnyFlags(RF_ClassDefaultObject | RF_ArchetypeObject | RF_NeedLoad | RF_NeedPostLoad))  // filter out CDO and ArchetypeObjects
    {
