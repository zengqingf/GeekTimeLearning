...
void FBuglyAgent::PostLogError( FString& name, FString& reason, FString& stackTrace)
{
	...
	NSString* Name = [NSString stringWithFString : name];
	NSString* Reason = [NSString stringWithFString : reason];
	NSString* StackTrace = [NSString stringWithFString : stackTrace];
	NSArray* StackArray = [NSArray arrayWithObject : StackTrace];
	NSDictionary* dic = [NSDictionary dictionary];
	[Bugly reportExceptionWithCategory : 3 name : Name reason:Reason callStack: StackArray  extraInfo: dic  terminateApp: NO] ;
}
