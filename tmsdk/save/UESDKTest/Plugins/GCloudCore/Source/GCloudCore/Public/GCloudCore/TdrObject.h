...
#ifndef ABASE_TDR_BASE_H_
#define ABASE_TDR_BASE_H_
...
const int TDR_OBJECT_VERSION = 1;
...
class ITdrObject
{
    ...
    public:
        ...
        virtual TdrError::ErrorType
        fromXmlFile(const char* file, TdrXmlFormat format = ABase::LIST_ENTRY, unsigned cutVer = 0);

        virtual TdrError::ErrorType
        fromXmlBuffer(const char* buffer, size_t size, TdrXmlFormat format = ABase::LIST_ENTRY, unsigned cutVer = 0);

        /* 'string' should be null-terminated and may be modified */
        virtual TdrError::ErrorType
        fromXmlString(char* string, TdrXmlFormat format = ABase::LIST_ENTRY, unsigned cutVer = 0);

        virtual TdrError::ErrorType
        toXmlFile(const char* file, TdrXmlFormat format = ABase::LIST_ENTRY, unsigned cutVer = 0) const;

        virtual TdrError::ErrorType
        toXmlFile(FILE* fp, TdrXmlFormat format = ABase::LIST_ENTRY, unsigned cutVer = 0) const;

        virtual TdrError::ErrorType
        toXmlBuffer(char* buffer, size_t size, size_t* usedSize = NULL,
                    TdrXmlFormat format = ABase::LIST_ENTRY, unsigned cutVer = 0) const;
