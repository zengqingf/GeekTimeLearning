...
#ifndef ABASE_TDR_BUF_H_
#define ABASE_TDR_BUF_H_
...
class TdrReadBuf
{
    ...
    void decode_64_var(const char* buf, size_t size, size_t& used, uint64_t& dst, int ret)
    {
        do {
            //(dst) = 0;
            (ret) = 0;
    ...
    TdrError::ErrorType readVarUInt64(uint64_t& dest)
        {
            ...
            int ret = 0;
            size_t used = 0;
            decode_64_var(beginPtr + position, length - position, used, dest, ret);
            if (ret != 0)
            {
                return TdrError::TDR_ERR_SHORT_BUF_FOR_READ;
            }
