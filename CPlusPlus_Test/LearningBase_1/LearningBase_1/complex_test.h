...
/*
友元friend
允许一个类将其非公有成员的访问权授予指定函数或类
声明以friend开始，只能出现在类定义的内部，可以放在内部的任何地方（友元不是授予友元关系的那个类的成员，所以它们不受其声明出现部分的访问控制影响）（通常放在类定义的开始或结尾）
...
1.友元函数：
	...
	没有this指针
	没有破坏继承机制（但会破坏封装性）
	不一定需要通过对象或者指针调用（静态函数也可以是友元函数）

2.友元类
	友元类的所有成员函数都是另一个类的友元函数，都可以访问另一个类中的隐藏信息（私有，保护；通过类对象访问类的所有成员）
	当一个类可以存取另一个类的私有成员时，可以将该类声明为另一类的友元类

	友元关系不能被继承
	友元关系是单向的，类B是类A的友元，A不一定是B的友元
	友元关系不存在传递性

3.友元成员函数
	使类B中的成员函数成为类A的友元函数，这样类B的成员函数就可以访问A的所有成员了
	一般，需要先定义包含成员函数的类，再将成员函数设为友元；
	不需要预先声明类和非成员函数来将它们设为友元


	应用：
	在和不支持类概念的语言（C）进行混合编程时，通过友元函数重载可以更自然使用C++的IO
*/
