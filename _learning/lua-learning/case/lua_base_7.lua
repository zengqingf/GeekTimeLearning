--[[
    ...
    copy from lua_base_forloop.lua
...
do
    ...
    --[[
        values作为工厂，每当它被调用的时候，就会创建一个新的闭包（迭代器本身）。
        这个闭包将自身的状态保存在外部变量"t"和"i"中，这两个变量是由values封装的。
        每次调用迭代器，i的值加1，然后返回t[i]，i作为迭代器的外部变量，在迭代器返回后不会被清除，其值得以保存下来，起到了记录迭代器位置的作用。
        在遍历完table的最后一个元素后，迭代器返回nil，表示迭代结束。
    ]]
