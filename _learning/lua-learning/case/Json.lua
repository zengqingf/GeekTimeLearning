...
function JSON:onDecodeError(message, text, location, etc)
    ...
    if etc ~= nil then
        message = message .. " (" .. self:encode(etc) .. ")"
    end

    if self.assert then
        self.assert(false, message)
    else
        print("收到的Json数据格式错误，请进行语法检查!")
...
function JSON:decode(text, etc)
    if type(self) ~= 'table' then
        print("Lua JSON decode failed. self is not table")
        return nil
    end

    if text == nil then
        print("Lua JSON decode failed. param text is nil")
        return nil
    ...
    if text:match('^%s*$') then
        print("Lua JSON decode failed. text:match fail")
        return nil
    end

    if text:match('^%s*<') then
        -- Can't be JSON... we'll assume it's HTML
        print("Lua JSON decode failed. text:match is html")
        return nil
    end
