...
* [Windows 批处理(bat)语法大全](https://www.cnblogs.com/zhaoqingqing/p/4620402.html)
...
* 设置win环境变量
  ...
  [使用WMIC永久设置你的环境变量](https://www.cnblogs.com/killerlegend/p/3405484.html)

  ``` powershell
  :: 使用wmic （WMIC扩展WMI（Windows Management Instrumentation，Windows管理规范），提供了从命令行接口和批命令脚本执行系统管理的支持。）
  :: 查看系统变量
...
* 文件拷贝
  ...
  ``` powershell
  copy /y d:\test\test.txt d:\       /y不加提示覆盖
  copy d:\test\test.txt d:\1.txt     相当于重命名
  
  复制的路径带有空格、&amp;等特殊符号的时候，请使用引号把这些特殊字符括起来
  
  合并多个文件为一个文件
  copy d:\test.txt+d:\abc.txt d:\test\test.txt  
  
  :: xcopy为copy的高级版本
  xcopy a:b:/s /e    驱动器 A 将所有文件和子目录（包括所有空的子目录）复制到驱动器 B
  xcopy a:b:/s /e /h   包含任何系统或隐藏文件
  xcopy \rawdata \reports /d:12-29-1993    \Rawdata 目录中 1993 年 12 月 29 日后更改的文件更新 \Reports 目录中的文件
  xcopy \rawdata \reports /u				 更新 \Reports 中存在的所有文件，而不考虑日期
  xcopy \rawdata \reports /d:12-29-1993 /l > xcopy.out		只获得前一个命令要复制的文件列表，而不复制文件
  xcopy \customer h:\public\address /s /e /k /p	
  将 \Customer 目录和所有子目录复制到网络驱动器 H: 上的目录 \\Public\Address，保留只读属性，并在 H: 上创建新文件时对您做出提示
  xcopy \customer h:\public\address /s /e /k /p /i
  要发出上一个命令，确保在 \Address 目录不存在时 xcopy 会创建该目录，同时应抑制显示创建新目录时要显示的消息，请按如下所示添加 /i
  ```
...
* bat安装python 并配置环境变量
  ...
  [Windows批处理.bat自动安装Python、设置系统变量、替换文件及安装库](https://blog.csdn.net/mighty13/article/details/78016027)
