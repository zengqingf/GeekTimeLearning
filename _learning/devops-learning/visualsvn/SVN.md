...
* SVN和Git的区别
  ...
  ``` tex
  控制系统：git是分布式（即个人电脑存在一份版本库），svn是集中式；
  版本库：svn只有一个中央版本库，而git有无限个；
  版本号：svn有全局的版本号，git没有；
  日志：git不必联网就可以看到所有的log，svn必须联网；
  存储方式：git保存的是元数据，svn是复制整个文档；
  分支：git强调分支，svn只是不同的文件目录，就是copy；
  内容完整性：git使用SHA-1哈希算法，确保内容完整性，磁盘故障和网络问题降低对版本库的破坏；
  速度：git速度优于svn
  ```
