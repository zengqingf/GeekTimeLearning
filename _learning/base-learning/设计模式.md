...
* State vs. Status
  ...
  ``` tex
  State:	如果「状态」的有效值之间可以搞出类似状态迁移图之类的东西，就命名为 state；否则就用 status
  	状态机： State Mechine  
  	状态迁移图：State Transition Diagram
  	TCP状态之间有迁移关系：Tcp State
  	
  Status:	
  	Http状态码 没有互相迁移关系：HTTP status code
  ```

  ![img](设计模式.assets/v2-709b9aa1d74f1f5e18f1563ef1ee3c28_720w.jpg)
